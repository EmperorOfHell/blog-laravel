<section class="bg-white py-8 antialiased w-2/3 rounded-md shadow-sm">
    <div class="px-4">
{{--        <div class="flex justify-between items-center mb-6">--}}
{{--            <h2 class="text-lg lg:text-2xl font-bold text-gray-900 dark:text-white">Discussion (20)</h2>--}}
{{--        </div>--}}
        @auth()
        <x-blogs::comments.form />
        @endauth
        <div class="[&>*:not(:last-child)]:mb-3 ">
        @foreach($comments as $id => $comment)
            <article class="bg-gray-100 duration-150 p-6 text-base  rounded-lg">
                <div class="flex justify-between items-center mb-2">
                    <div class="flex items-center">
                        <p class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white font-semibold">
                                {{$comment->user->name}}
                        </p>
                            <x-blogs::comments.static-rating :rating="$comment->rating"/>

                    </div>
                    @if(auth()->id() === $comment->user->id)
                        <div x-data="{dropdown{{$id}}Show: false}" class="relative">


                            <button @click="dropdown{{$id}}Show = true"
                                    class="inline-flex items-center p-2 text-sm font-medium text-center text-gray-500 rounded-lg hover:bg-white focus:bg-white focus:outline-none "
                                    type="button">
                            ...
                            <div
                                x-cloak
                                x-show="dropdown{{$id}}Show"
                                @keydown.escape="dropdown{{$id}}Show = false"
                                @click.away="dropdown{{$id}}Show = false"
                                 class=" absolute z-10 mt-2 w-36 rounded-md shadow-lg bg-white right-0">
                ...
                <p class="text-gray-500 mb-2">{{$comment->body}}</p>
